    ...
    <template #button>
      <div class="bg-white border border-gray-300 px-4 py-3 text-sm text-gray-500">
        <span class="text-gray-900 font-medium">{{
          $dayjs($stores.statistics.params.startDate).format('DD/MM/YYYY')
        }}</span>
        -
        <span class="font-medium text-gray-900">{{
          $dayjs($stores.statistics.params.endDate).format('DD/MM/YYYY')
        }}</span>
      </div>
    </template>
    ...
    <template #items>
      <div class="py-4 px-6 space-y-4">
        <div class="flex spaxe-y-4 gap-4">
          <div class="w-[180px]">
            <div class="text-gray-500 mb-4 font-semibold">Préfiltres</div>
          ...
          <div>
            <div class="text-gray-500 mb-4 font-semibold">
              À partir de
              <span class="text-gray-800">{{ $dayjs(form.startDate).format('DD/MM/YYYY') }}</span>
            </div>
            <v-date-picker
              ref="calendarStart"
              v-model="form.startDate"
              :attributes="attributesStartDate"
            />
          </div>
          <div>
            <div class="text-gray-500 mb-4 font-semibold">
              Jusqu'au
              <span class="text-gray-800">{{ $dayjs(form.endDate).format('DD/MM/YYYY') }}</span>
            </div>
            <v-date-picker
              ref="calendarEnd"
        ...
        <div class="flex justify-end">
          <BaseButton @click.native="onSubmit"> Appliquer </BaseButton>
        </div>
      </div>
    </template>
  </BaseDropdown>
    ...
    async onFilterClick(filter) {
      ...
      switch (filter) {
        ...
        case 'last_year':
          ...
          await calendarStart.move({
            month: 1,
            year: this.$dayjs().subtract(1, 'year').year(),
          })
          await calendarEnd.move({
            month: 12,
            year: this.$dayjs().subtract(1, 'year').year(),
        ...
        case 'last_month':
          ...
          await calendarStart.move({
            month: this.$dayjs().subtract(1, 'month').month() + 1,
            year: this.$dayjs().subtract(1, 'month').year(),
          })
          await calendarEnd.move({
            month: this.$dayjs().subtract(1, 'month').month() + 1,
            year: this.$dayjs().subtract(1, 'month').year(),
        ...
        case 'last_week':
          ...
          await calendarStart.move({
            month: this.$dayjs().subtract(1, 'week').month() + 1,
            year: this.$dayjs().subtract(1, 'week').year(),
          })
          await calendarEnd.move({
            month: this.$dayjs().subtract(1, 'week').month() + 1,
            year: this.$dayjs().subtract(1, 'week').year(),

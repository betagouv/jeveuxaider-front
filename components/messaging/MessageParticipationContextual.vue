<template>
  <div class="text-center">
    ...
    <div v-if="badge">
      ...
      <DsfrBadge size="sm" :type="badge.type" :no-icon="true" class="ml-2">
        {{ badge.label }}
      </DsfrBadge>
      <span v-if="badge.append" class="ml-2 text-[#161616] font-bold">{{ badge.append }}</span>
    </div>
    <div
      v-if="message.contextual_reason && message.contextual_reason != 'other'"
      class="text-sm text-[#666666]"
    >
      <template v-if="message.contextual_state == 'Refusée'">
        {{ $filters.label(message.contextual_reason, 'participation_declined_reasons') }}
      </template>
      <template v-if="message.contextual_state == 'Annulée par bénévole'">
        ...
        <template v-else>
          {{
            $filters.label(message.contextual_reason, 'participation_canceled_by_benevole_reasons')
          }}
        </template>
      </template>
  ...
  computed: {
    badge() {
      switch (this.message.contextual_state) {
        case 'Refusée':
          return {
            prepend: 'La participation a été',
            type: 'error',
            label: 'Refusée',
          }
        case 'Validée':
        ...
        case 'Automatiquement déclinée par la plateforme':
          return {
            prepend: 'La participation a été automatiquement',
            type: 'error',
            label: 'Refusée',
            append: 'par la plateforme',
          }
        default:
          return {
            prepend: 'La participation a été',
            type: 'warning',
            label: this.message.contextual_state,
          }
      }
    },
  },

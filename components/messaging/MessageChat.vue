...
export default {
  ...
  methods: {
    linkify (inputText) {
      ...
      let replacedText = inputText.replace(replacePattern1, '<a href="$1" target="_blank" class="break-all underline hover:text-gray-700">$1</a>')

      // URLs starting with www. (without // before it, or it'd re-link the ones done above)
      const replacePattern2 = /(^|[^/])(www\.[\S]+(\b|$))/gim
      replacedText = replacedText.replace(replacePattern2, '$1<a href="http://$2" target="_blank" class="break-all underline hover:text-gray-700">$2</a>')

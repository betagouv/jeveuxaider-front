...
export default {
  ...
  methods: {
    linkify (inputText) {
      ...
      let replacedText = inputText.replace(replacePattern1, '<a href="$1" target="_blank" class="break-all hover:underline">$1</a>')

      // URLs starting with www. (without // before it, or it'd re-link the ones done above)
      const replacePattern2 = /(^|[^/])(www\.[\S]+(\b|$))/gim
      replacedText = replacedText.replace(replacePattern2, '$1<a href="http://$2" target="_blank" class="break-all hover:underline">$2</a>')

      // Change email addresses to mailto:: links
      // const replacePattern3 = /(([a-zA-Z0-9_\-.]+)@[a-zA-Z_]+?(?:\.[a-zA-Z]{2,6}))+/gim
      // replacedText = replacedText.replace(replacePattern3, '<a href="mailto:$1" class="break-all hover:underline">$1</a>')

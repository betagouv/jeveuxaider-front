<template>
  <div v-click-outside="() => (isGeolocFilterActive = false)">
    ...
    <button
      ...
      ref="fakeInput"
      class="flex space-x-3 items-center justify-center text-gray-900 truncate w-full"
      @click="handleFakeInputClick"
    >
      <RiMapPin2Line class="h-[20px] w-auto flex-none fill-current text-gray-900" />
      <div
      ...
      <div v-else-if="!$route.query.city" class="truncate font-bold text-gray-900">
        Ville ou code postal
      </div>
      <div v-else class="font-bold truncate">
        {{ $route.query.city }}
      </div>
      <RiArrowDownSLine class="h-5 w-5 text-gray-900 fill-current self-end" />
    </button>

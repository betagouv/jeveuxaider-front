<template>
  <Box padding="sm" :loading="loading" loading-text="Générations des données...">
    <BoxHeadingStatistics title="Référents ›" link="/support/referents/waiting-actions" no-period class="mb-6" />
    <div v-if="overview" class="grid grid-cols-1 lg:grid-cols-4 border bg-gray-200 gap-[1px] overflow-hidden">
      <CardStatistic
        ...
        :subtitle="`sur ${$options.filters.formatNumber(overview.referents_count)} référents`"
      />
      <CardStatistic
...
export default {
  ...
  async fetch () {
    ...
    await this.$axios.get('/support/referents/overview').then((response) => {
      this.loading = false

<template>
  <div>
    <Box padding="sm" :loading="loading" loading-text="Récupération des participations...">
      <BoxHeadingStatistics title="Participations annulées" class="mb-6" />
      <StackedList v-if="items" :divided="false">
        <StackedListItem
          ...
          :link="`/admin/participations?filter[mission.structure.name]=${item.name}&filter[mission.structure.id]=${item.id}&filter[state]=Annulée`"
        >
...
export default {
  ...
  async fetch () {
    this.loading = true
    await this.$axios.get('/statistics/participations-canceled-by-organisations', {

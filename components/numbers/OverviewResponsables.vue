<template>
  <BaseBox padding="sm" :loading="loading" loading-text="Générations des données...">
    <BoxHeadingStatistics
      title="Responsables ›"
      link="/support/responsables/participations-to-be-treated"
      no-period
      class="mb-6"
    />
    <div
    ...
    >
      <CardStatistic
        ...
        :subtitle="`sur ${$numeral(overview.responsables_count)} responsables`"
      />
      <CardStatistic
...
export default defineNuxtComponent({
  ...
  methods: {
    async fetch() {
      ...
      await apiFetch('/support/responsables/overview').then((response) => {
        this.loading = false

<template>
  <Box padding="sm" :loading="loading" loading-text="Générations des données...">
    ...
    <div v-if="statistics" class="flex flex-col gap-2">
      <ListItemCount color="draft" label="Brouillon" :count="statistics.draft" :total="total" display="count_percent" />
      ...
      <ListItemCount color="signaled" label="Signalée" :count="statistics.signaled" :total="total" display="count_percent" />
      <ListItemCount color="unsubscribed" label="Désinscrite" :count="statistics.signaled" :total="total" display="count_percent" />
    </div>
  </Box>
</template>
...
export default {
  ...
  async fetch () {
    this.loading = true
    await this.$axios.get('/statistics/public/organisations-by-states', {

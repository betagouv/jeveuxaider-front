<template>
  <BaseBox padding="sm" :loading="loading" loading-text="Générations des données...">
    <BoxHeadingStatistics
      ...
      class="mb-6"
      infos-bulle="Répartition des missions validées, créées sur la période, en fonction de l’utilisation d’un modèle de mission"
    />
    <div class="w-full">
      <DoughnutChart
        v-if="chartData"
...
export default defineNuxtComponent({
  ...
  methods: {
    async fetch() {
      this.loading = true
      await apiFetch('/statistics/public/missions-by-template-types', {

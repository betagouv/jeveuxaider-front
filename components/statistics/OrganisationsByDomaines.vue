<template>
  <BaseBox padding="sm" :loading="loading" loading-text="Générations des données...">
    <BoxHeadingStatistics
      title="Répartition des organisations par domaine d’action"
      class="mb-6"
      infos-bulle="Répartition des organisations inscrites et validées sur la période par domaine d'action"
    />
...
export default defineNuxtComponent({
  ...
  methods: {
    async fetch() {
      this.loading = true
      await apiFetch('/statistics/public/organisations-by-domaines', {

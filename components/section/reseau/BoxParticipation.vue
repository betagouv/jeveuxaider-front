<template>
  <div>
    <div class="text-sm flex justify-between px-2 mb-2 items-center">
      ...
      <Link :to="`/admin/participations?filter[ofReseau]=${reseau.id}&reseau_name=${reseau.name}`" icon="ChevronRightIcon">
        Consulter
      </Link>
    </div>
    <Box variant="flat" padding="xs" :loading="!stats">
      <template v-if="stats">
        ...
        <template v-if="stats.participations_state['En attente de validation'] > 0">
          ...
          <nuxt-link :to="`/admin/participations?filter[ofReseau]=${reseau.id}&reseau_name=${reseau.name}&filter[state]=En attente de validation`" class="flex justify-center items-center text-sm text-jva-orange-500 font-medium">

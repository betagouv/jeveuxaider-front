<template>
  <div>
    <div v-if="$store.state.algoliaSearchOrganisations.results" class="container md:px-8 lg:mt-6 mb-12">
      <div class="flex flex-col space-y-6 sm:space-y-12">
        ...
        <div v-if="$store.state.algoliaSearchOrganisations.results.nbHits == 0" class="text-center">
        ...
        <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-8 lg:gap-6 xl:gap-8 xl:max-w-5xl mx-auto">
          <template v-for="item in $store.state.algoliaSearchOrganisations.results.hits">
            <nuxt-link
              :key="item.id"
        ...
        <Pagination
          :current-page="$store.state.algoliaSearchOrganisations.results.page + 1"
          :total-rows="$store.state.algoliaSearchOrganisations.results.nbHits"
          :per-page="20"
          :max-pages="10"

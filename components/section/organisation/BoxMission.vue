<template>
  <div>
    <div class="text-sm flex justify-between px-2 mb-2 items-center">
      ...
      <Link :to="`/admin/missions?filter[structure.id]=${organisation.id}&context_name=${organisation.name}`" icon="ChevronRightIcon">
        Consulter
      </Link>
    </div>
        ...
        <div class="flex items-center">
          ...
          <div>
            <div class="font-semibold">
              {{ organisationStats.missions_available | pluralize('mission', 'missions', false) }} en ligne
            </div>
            <div class="text-gray-500 -mt-1">
              sur {{ organisationStats.missions_total| pluralize('mission') }}
            </div>
          </div>
        </div>

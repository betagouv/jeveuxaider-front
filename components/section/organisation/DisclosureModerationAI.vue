...
export default {
  ...
  computed: {
    sentencesWithKeyAndScore () {
      ...
      return this.$store.state.aideModeration.response.sentences.map((sentence, key) => ({ key, text: sentence, score: this.$store.state.aideModeration.response.scores[key] }))
    },
    sentencesSuccess () {

<template>
  <Disclosure>
    ...
    <div v-if="response" class="ml-7 mt-3 text-sm space-y-2 text-gray-500">
      <div> Score de conformité : {{ score | formatNumber("0.[00]") }}%. En dessous de 87%, l'organisation présente un risque.</div>
      ...
      <div class="text-xs italic">
        Cette information vient de notre modèle d’intelligence artificielle, qui se base sur l’historique des organisations signalées. Il s’agit d’un outil en test, une vérification humaine est nécessaire.
      </div>
    </div>
  </Disclosure>
</template>

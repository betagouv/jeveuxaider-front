  ...
  <div v-if="territoire.seo_engage_paragraphs">
    ...
    <section class="section-associations bg-[#fafaff]">
      <div class="container mx-auto px-4 py-12">
        ...
        <div v-for="(item, key) in territoire.seo_engage_paragraphs" :key="item.uuid">
          <hr />
          ...
          >
            <h3 class="lg:col-span-2 text-2xl font-extrabold tracking-[-1px]" v-html="item.title" />

            <BaseTextFormatted class="lg:col-span-3 tracking-tight" :text="item.description" />
          </div>
        </div>
      </div>
    </section>
...
export default defineNuxtComponent({
  ...
  computed: {
    last() {
      return Object.keys(this.territoire.seo_engage_paragraphs).length - 1
    },
  },
})
</script>

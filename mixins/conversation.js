export default {
  computed: {
    ...
    hasUnreadMessage() {
      ...
      if (
        this.$dayjs(this.currentUserInConversation?.pivot.read_at).isSame(
          this.$dayjs(this.conversation.updated_at)
        )
      ) {
        return false
      }
      if (
        this.$dayjs(this.currentUserInConversation?.pivot.read_at).isAfter(
          this.$dayjs(this.conversation.updated_at)
        )
      ) {
        return false
    ...
    getPlaceMission(conversation) {
      const model =
        conversation.conversable_type == 'App\\Models\\Participation'
          ? conversation.conversable.mission
